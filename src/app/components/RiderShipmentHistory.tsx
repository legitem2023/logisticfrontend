...
// Proof of Pickup Card Component
const ProofOfPickupCard = ({ proof }: { proof: any }) => (
...
const ProofOfDeliveryCard = ({ proof }: { proof: any }) => (
  <div className="bg-white rounded-xl p-5 border border-gray-200 shadow mb-4">
    <div className="flex items-center gap-2 text-gray-700 font-semibold text-sm uppercase tracking-wider border-b border-gray-200 pb-2 mb-4">
      <Truck className="h-5 w-5 text-green-500" />
      DELIVERY PROOF - {formatDate(proof.receivedAt)}
    </div>

    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
      <div className="space-y-4">
        <div>
          <div className="text-xs text-gray-500 mb-1">Received By</div>
          <div className="font-medium">{proof.receivedBy || 'N/A'}</div>
        </div>
        
        <div>
          <div className="text-xs text-gray-500 mb-1">Received At</div>
          <div className="font-medium">{formatDate(proof.receivedAt)}</div>
        </div>
      </div>
      ...
      <div className="space-y-4">
        {proof.photoUrl && (
          <div>
            <div className="text-xs text-gray-500 mb-2">Delivery Photo</div>
            <div className="relative h-48 w-full rounded-lg overflow-hidden border">
              <Image
                src={proof.photoUrl}
        ...
        {proof.signatureData && (
          <div>
            <div className="text-xs text-gray-500 mb-2">Recipient Signature</div>
            <div className="relative h-24 w-full bg-gray-50 rounded-lg p-2 border border-gray-200">
              <Image
      ...
      <div className="relative p-1 space-y-4">
        ...
        {filteredDeliveries.length === 0 && !loading && (
          <div className="text-center py-12">
            ...
            <p className="mt-1 text-sm text-gray-500">No shipments found for this status.</p>
          </div>
        )}
        ...
        <div
          className={`fixed bottom-0 left-0 w-full bg-white shadow-t-lg transform transition-transform duration-300 ease-in-out z-50
            ${drawerOpen ? "translate-y-0" : "translate-y-full"}
            h-[85vh] rounded-t-2xl border-t overflow-hidden flex flex-col
          `}
        >
          <div className="flex justify-between items-center p-4 border-b bg-white">
            <h2 className="text-lg font-semibold text-gray-900">Shipment Details</h2>
          ...
          {selectedDelivery && (
            <div className="flex-1 overflow-y-auto">
              <div className="relative bg-gradient-to-br from-white to-green-50 min-h-full p-4">
                <div className="absolute top-0 left-0 w-32 h-32 bg-green-200 rounded-full filter blur-[100px] opacity-40"></div>
                <div className="absolute bottom-0 right-0 w-48 h-48 bg-green-100 rounded-full filter blur-[100px] opacity-40"></div>
                ...
                <div className="p-6 border-b border-green-200 bg-gradient-to-r from-white to-green-50 rounded-t-2xl">
                  <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <h2 className="text-xl font-bold text-gray-900">Shipment Details</h2>
                        <p className="text-xs text-green-600">Complete delivery information</p>
                      </div>
                    </div>
                ...
                <div className="p-6 space-y-6 text-gray-700">
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      ...
                      />
                      <DetailCard 
                        icon="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 01118 0z"
                  ...
                  {selectedDelivery.proofOfPickup && selectedDelivery.proofOfPickup.length > 0 && (
                    <div className="space-y-4">
                      ...
                      {selectedDelivery.proofOfPickup.map((proof: any, index: number) => (
                        <ProofOfPickupCard key={index} proof={proof} />
                      ))}
                    </div>
                  )}
                  ...
                  {selectedDelivery.proofOfDelivery && selectedDelivery.proofOfDelivery.length > 0 && (
                    <div className="space-y-4">
                      ...
                      {selectedDelivery.proofOfDelivery.map((proof: any, index: number) => (
                        <ProofOfDeliveryCard key={index} proof={proof} />
                      ))}
                    </div>
                  )}
