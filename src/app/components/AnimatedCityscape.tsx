...
const AnimatedCityscape = ({ theme = 'night', speed = 1 }) => {
  ...
  useEffect(() => {
    ...
    buildings.forEach(building => {
      ...
      windows.forEach(window => {
        const brightness = theme === 'night' 
          ? Math.random() * 0.7 + 0.3 
          : Math.random() * 0.3 + 0.7;
        window.style.opacity = brightness;
        
        if (theme === 'night') {
          const flicker = setInterval(() => {
            const variation = Math.random() * 0.3;
            window.style.opacity = Math.max(0.1, brightness - variation);
    ...
    if (theme === 'night' && starsRef.current) {
      ...
      stars.forEach(star => {
        ...
        star.style.animation = `twinkle ${duration}ms ease-in-out ${delay}ms infinite alternate`;
      });
    }
  }, [theme, speed]);
        ...
        for (let col = 0; col < windowsPerRow; col++) {
          if (Math.random() > 0.2) {
            windows.push(
              <div 
                key={`window-${row}-${col}`} 
                className="window absolute bg-y-500 rounded-sm"
                style={{
  ...
  const generateCars = () => {
    ...
    for (let i = 0; i < carCount; i++) {
      const carRef = el => carsRef.current[i] = el;
