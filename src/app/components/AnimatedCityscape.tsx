...
<AnimatedCityScape className="h-[380px]"><YourHeader /></AnimatedCityScape>
/ export default function AnimatedCityScape({ className, children, }: { className?: string; children?: React.ReactNode; }) { return ( <div className={clsx( "relative w-full overflow-hidden", // Sky background "bg-gradient-to-b from-sky-300 via-sky-200 to-sky-100", // Optional height; override via className "h-[320px] md:h-[420px]", className )} > {/ Sun / glow */} <div className="pointer-events-none absolute -top-10 right-10 h-40 w-40 rounded-full bg-yellow-300/70 blur-2xl" />
...
a seamless loop. Width is 200% so translateX(-50%) loops perfectly. */ function ParallaxStrip({ className, speedClass, buildingTone, heights, hasAntennas = false, }: { className?: string; speedClass: string; // e.g., "animate-[scrollX_40s_linear_infinite]" buildingTone: string; // tailwind gradient classes, e.g., "from-slate-500 to-slate-700" heights: number[]; // building heights in px hasAntennas?: boolean; }) { return (
...
function BuildingsRow({ buildingTone, heights, hasAntennas, }: { buildingTone: string; heights: number[]; hasAntennas?: boolean; }) { return ( <div className="flex w-1/2 items-end gap-2 px-2"> {heights.map((h, i) => ( <div key={${h}-${i}} className="relative flex items-end"> {/* Building /} <div className={clsx( "relative w-10 rounded-t-sm bg-gradient-to-b shadow-sm", buildingTone )} style={{ height: ${h}px }} > {/ Simple windows using repeating linear gradient overlay */} <div className="absolute inset-0 opacity-25 mix-blend-overlay" style={{ backgroundImage: "repeating-linear-gradient(transparent, transparent 18px, rgba(255,255,255,0.9) 18px, rgba(255,255,255,0.9) 20px)", }} />
