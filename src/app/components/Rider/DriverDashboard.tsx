  ...
  return acceptedDeliveries.filter((delivery: any) => {
    const matchesSearch = searchTerm 
      ? delivery.trackingNumber.toLowerCase().includes(searchTerm.toLowerCase()) ||
        ...
        (delivery.sender?.name?.toLowerCase().includes(searchTerm.toLowerCase()) ?? false)
      : true;
     console.log(new Date(delivery.createdAt).toDateString());
     console.log(new Data(selectedDate).toDateString());
                  ...
                  <CardContent className="flex flex-col justify-between items-start sm:items-center gap-4 p-4">
                    <div>
                      ...
                      <div className="flex items-center gap-2 text-sm text-gray-500 mt-2">
                        ...
                        <span className="italic">ATA: {convertMinutesToHours(parseInt(delivery.ata==="" || delivery.ata===null?"0":delivery.ata))}</span>
                      </div>
                    </div>
      ...
      {showProof && (
       <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center bg-black/40 backdrop-blur-sm transition-all">
         ...
         <div className="flex justify-between items-center p-4 border-b bg-white">
           ...
           <ProofOfDeliveryForm data={{id:deliveryId}} refresh={refetch}/>
         </div>
       </div>
      )}
    ...
    {showPickupProof && (
       <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center bg-black/40 backdrop-blur-sm transition-all">
         ...
         <div className="flex justify-between items-center p-4 border-b bg-white">
           ...
           <PickupProofForm data={{id:deliveryId}} refresh={refetch}/>
         </div>
       </div>
      )}
