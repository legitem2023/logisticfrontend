...
import { LOGOUT_MUTATION, FBLOGIN, LOGIN, GOOGLELOGIN } from "../../graphql/mutation";
  ...
  callbacks: {
    async jwt({ token, account, user }) {
      ...
      if (account?.provider === "google") {
        console.log("Google account detected, processing...");
        ...
        try {
          console.log("Attempting GraphQL mutation for Google login...");
          console.log("Server URL:", process.env.NEXT_PUBLIC_SERVER_LINK);
          
          const { data } = await client.mutate({
            mutation: GOOGLELOGIN, // You'll need to add this mutation
            variables: {
              input: {
                idToken: idToken,
              },
          ...
          if (data?.loginWithGoogle?.token) {
            ...
            token.serverToken = data.loginWithGoogle.token as string;
          } else {
      ...
      // Handle credentials login
        ...
        console.log("Account provider is not Google/Facebook or account is missing:", account?.provider);
      }
